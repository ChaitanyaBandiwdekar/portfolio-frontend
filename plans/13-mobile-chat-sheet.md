...
- [x] **Add `expanded` state to `ChatSection`.** When true, the `<div ref={windowRef}>` wrapper
...
- [x] **Suppress the CRT clip-path animation while expanded.** `ChatSection.tsx:19-37` animates
...
- [x] **Open on input focus.** In `Terminal.tsx`, add an `onExpand` prop; call it from the input's
...
- [x] **Sheet chrome.** Reuse `TerminalWindow`'s titlebar vocabulary. Header row: the robot at ~44px
      on the left, the title, and a `✕` close button with a ≥44px target on the right.

- [x] **Robot in the sheet.** `RobotScene` already handles a compact layout via `compactLayout`
...
- [x] **Exit paths (both, per decision):**
...
- [x] **Scroll lock.** Reuse the exact idiom from `Nav.tsx:46-47`: `document.body.style.overflow =
...
- [x] **Docked input.** In the sheet the form (`Terminal.tsx:203`) sits at the bottom of a flex
...
- [x] **Fix the scroll-to-bottom yank.** `Terminal.tsx:51-56` sets `scrollTop = scrollHeight` on
...
- [x] **Message metrics (mobile + sheet).** Keep the bot container as the bot/user distinction, but:
...
- [x] **Shrink the inline preview.** With the sheet carrying real conversation, the inline transcript
